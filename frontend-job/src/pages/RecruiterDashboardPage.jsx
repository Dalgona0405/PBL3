...
function RecruiterDashboardPage() {
    ...
    useEffect(() => {
        ...
        const fetchJobs = async () => {
            try {
                ...
                setRecruiterInfo(data); // Lưu lại info để biết HR này có công ty chưa
                setJobs(data.jobs ||[]); 
    ...
    // Hàm xử lý Xóa Job (Tạm thời để console.log, mình sẽ nối API sau)
    const handleDeleteJob = (jobId) => {
        if(window.confirm("Bạn có chắc chắn muốn xóa tin tuyển dụng này không?")) {
            console.log("Sẽ gọi API DELETE /api/jobs/", jobId);
            alert("Chức năng xóa đang được xây dựng 🌿");
        }
    };
